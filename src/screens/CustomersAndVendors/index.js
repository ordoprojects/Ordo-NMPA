...
const Drivers = ({ fetchData, pendingOrders, loading, renderItem, navigation }) => {
    ...
    return (
        <View style={{ flex: 1, backgroundColor: 'white' }}>
            ...
            <FlatList
                ...
                ListEmptyComponent={() => (
                    <View style={styles.noProductsContainer}>
                        <Text style={styles.noProductsText}>No Drivers Orders</Text>
                    </View>
                )}
            ...
            {/* <AnimatedFAB
                label={'Add Customer'}
                icon={() => <FontAwesome5 name="user-plus" size={24} color="white" />}
                color={"white"}
                ...
                // iconMode={'static'}
                onPress={() => {
                    navigation.navigate("Add Customer", { screen: 'add' });
                }}
            /> */}
        </View>
    );
...
const Vehicles = ({ navigation, route, fetchData, vendors, renderItem, loading }) => {
            ...
            <FlatList
                ...
                renderItem={({ item }) => (
                    <TouchableOpacity
                        ...
                        activeOpacity={0.8}
                        onPress={() =>
                            navigation.navigate('SupplierDetails', { item })
                        }
                    >
                        <View
                            style={{
                ...
                ListEmptyComponent={() => (
                    <View style={styles.noProductsContainer}>
                        <Text style={styles.noProductsText}>No Vehicles Orders</Text>
                    </View>
                )}
            //keyExtractor={(item) => item.id.toString()}
            />
...
const CustomersAndVendors = ({ navigation }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                const vendorResponse = await fetch("https://gsidev.ordosolution.com/api/suppliers/", requestOptions);
                const vendorResult = await vendorResponse.json();
                console.log("vendor Data", vendorResult);
                setVendors(vendorResult);
                setFilteredVendors(vendorResult);

                const customerResponse = await fetch("https://gsidev.ordosolution.com/api/customer/", requestOptions);
                const customerResult = await customerResponse.json();
                // console.log("customer Data", customerResult);
                setCustomers(customerResult);
                setFilteredCustomers(customerResult);
    ...
    const handleFilter = () => {
        if (selectedOption === "all") {
            ...
            const filteredconfirmedorders = customers.filter((order) => {
                const orderDate = moment(order.created_at).format("YYYY-MM-DD");
                return orderDate === selectedDate;
            });
            setFilteredCustomers(filteredconfirmedorders);

            const filteredtransitorders = vendors.filter((order) => {
                const orderDate = moment(order.created_at).format("YYYY-MM-DD");
                return orderDate === selectedDate;
            });
            setFilteredVendors(filteredtransitorders);
      ...
      const applySearchFilter = (dataArray, setFilteredArray) => {
        ...
        if (search) {
          const newData = dataArray.filter(item => {
            const itemData = (item.name ? item.name.toUpperCase() : '') + (item.full_name ? item.full_name.toUpperCase() : '');
        ...
        >
          ...
          >
            ...
            <Tab.Navigator
                ...
                <Tab.Screen
                    name="Drivers"
                    options={{ title: 'Customers', tabBarLabelStyle: { fontFamily: "AvenirNextCyr-Bold", textTransform: 'capitalize' } }}
                >
                    {(props) => <Drivers {...props} pendingOrders={filteredCustomers} renderItem={renderItem} loading={loading} navigation={navigation} />}
                </Tab.Screen>
                ...
                <Tab.Screen
                    name="Vehicles"
                    options={{ title: 'Vendors', tabBarLabelStyle: { fontFamily: "AvenirNextCyr-Bold", textTransform: 'capitalize' } }}
                >
                    {(props) => <Vehicles {...props} vendors={filteredVendors} renderItem={renderItem} loading={loading} navigation={navigation} />}
                </Tab.Screen>
...
export default CustomersAndVendors
