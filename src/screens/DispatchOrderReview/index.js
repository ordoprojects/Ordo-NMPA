...
const Pending = React.memo(({ pendingArray, navigation, fetchData, renderItem,handleLoadMore,loading}) => {
...
const Delivered = React.memo(({ completedArray, navigation, ordersLoading, renderItem,handleLoadMore,loading }) => {
    if (ordersLoading) {
        return (
            <OrdersSkeleton />
        );
...
const Canceled = React.memo(({ cancelledArray, navigation, renderItem,handleLoadMore,loading }) => {
...
const DispatchOrderReview = ({ navigation,route}) => {
    ...
    const fetchPendingData = async () => {
        ...
        try {
            // if (screenName === 'Pending'){
                const PData = await fetchDataForTeam('Stock Approved', 10, 0, userData, search);
                setPendingArray(PData.results);
                setFilteredPendingArray(PData.results);

            // }else if (screenName === 'Delivered'){
                const CData = await fetchDataForTeam('Confirmed', 10, 0, userData, search);
    ...
    const fetchData = async () => {
        ...
        try {
            if (screenName === 'Pending'){
            const pendingData = await fetchDataForTeam('Stock Approved', 10, 0, userData, search);
            setPendingArray(pendingData.results);
            setFilteredPendingArray(pendingData.results);
            }
            else if(screenName === 'Delivered'){
            const completedData = await fetchDataForTeam('Confirmed', 10, 0, userData,search);
    ...
    const handleLoadMore = async ()=>{
        ...
        try {
            if (screenName === 'Pending'){
            const newPendingData = await fetchDataForTeam('Stock Approved', 10, pendingOffset, userData, '');
            ...
            const newCompletedData = await fetchDataForTeam('Confirmed', 10, approvedOffset, userData, '');
    ...
    const renderItem = ({ item }) => {
        ...
        return (
            <TouchableOpacity
                ...
                activeOpacity={0.8}
                onPress={() => {
                    navigation.navigate("DispatchOrderDetails", { orderDetails: item, screen: 'SO' });
