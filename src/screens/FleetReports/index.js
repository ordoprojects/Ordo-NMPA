...
const FleetReports = ({ navigation }) => {
    const { userData } = useContext(AuthContext)

    const [barType, setBarType] = useState('total_paid_amount')
    const [areaType, setAreaType] = useState('petrol')
    ...
    // forAreaChart
    useEffect(() => {
        if (areaChartData && areaType && areaMode && areaChartData[areaType] && areaChartData[areaType][areaMode]) {
    ...
    useEffect(() => {
        if (barChartData && Object.keys(barChartData).length > 0 && barType && barMode) {
            const data = barChartData[barMode];
    ...
    const fetchData = async () => {
        ...
        try {
            const response = await fetch("https://gsidev.ordosolution.com/api/fleet_dashboard/", requestOptions);
            const data = await response.json();

            const po_data = data.piechart.vehicle_types.map((item) => ({
                label: item.label,
    ...
    return (
        ...
        >
            ...
            <ScrollView style={styles.whiteView}>
                <Graphs pieData={pieChartData} pieChart={true} pieChartHeading="Vehicles" barChart={true} barGraphHeading="Maintainence Costs" barData={filteredBarData} navigation={navigation} moduleName="Fleet" areaChart={true} areaChartData={filteredAreaData}  >
