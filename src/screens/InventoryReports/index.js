...
const InventoryReports = ({ navigation }) => {
    ...
    const fetchData = async () => {
        ...
        try {
            const response = await fetch("https://gsidev.ordosolution.com/api/inv_warehouse_dashboard/", requestOptions);
            const data = await response.json();

            const po_data = data.warehouse.warehouse_utilization.map((item) => ({
                label: item.label,
            ...
            setPos(po_data);
            // setRfqs(rfq_data);
            setBarChartData(data?.inventory?.top_5_product_current_month)
            // setAreaChartData(data.inventory)
        } catch (error) {
...
<Text> </Text>
            ...
            <ScrollView style={styles.whiteView}>
                <Graphs pieData={pieChartData} pieChart={true} pieChartHeading="Warehouses     " barChart={true} barGraphHeading="Top Products" barData={barChartData} navigation={navigation}   >
