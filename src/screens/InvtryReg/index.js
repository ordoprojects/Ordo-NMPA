...
const InventoryReg = ({ navigation, visible, extended }) => {
  ...
  const fetchData = async () => {
    ...
    await fetch("https://gsidev.ordosolution.com/api/inventory_log/", requestOptions)
      ...
      .then((result) => {
        ...
        setFilteredPurchaseOrders(result.purchase_orders)
  ...
  useEffect(() => {
    ...
    applySearchFilter(salesOrders, setFilteredSalesOrders);
    applySearchFilter(purchaseOrders, setFilteredPurchaseOrders);
  

  }, [salesOrders, purchaseOrders,search]);

  const clearFilters = () => {
    setSelectedOption("all");
    setFilteredSalesOrders(confirmedOrders);
    setFilteredPurchaseOrders(transitOrders);
    // setFilteredDeliveredOrders(deliveredOrders);
    // setFilteredCancelledOrders(cancelledOrders);
  ...
  const renderItem = ({ item }, type) => {
    ...
    return (
      <TouchableOpacity
        onPress={() => { navigation.navigate('TrackingDetails', { orderDetails: item, screen: type }) }}
  ...
  return (
    ...
    >
      ...
      <Tab.Navigator
        ...
        <Tab.Screen
          name="Purchase"
          options={{ title: 'Purchase', tabBarLabelStyle: { fontFamily: "AvenirNextCyr-Bold", textTransform: 'capitalize' } }}
        >
          {(props) => <Purchase {...props} fetchData={fetchData} purchaseOrders={filteredPurchaseOrders} renderItem={renderItem} loading={loading}
            onScroll={onScroll}
        ...
        >
          {(props) => <Sales {...props} fetchData={fetchData} salesOrders={filteredSalesOrders} renderItem={renderItem} loading={loading}
            onScroll={onScroll}
          />}
        </Tab.Screen>
        ...
        <Tab.Screen
          name="Transfer"
          options={{ title: 'Transfer', tabBarLabelStyle: { fontFamily: "AvenirNextCyr-Bold", textTransform: 'capitalize' } }}
        >
          {(props) => <Transfer {...props} fetchData={fetchData} transitOrders={filteredTransferOrders} renderItem={renderItem} loading={loading}
            onScroll={onScroll}
