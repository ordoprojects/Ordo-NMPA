...
import PDFViewer from "../SMAprrovedPlanDetails/pdfViewer";
import { useFocusEffect } from "@react-navigation/native";


const ListInspections = ({ navigation }) => {
    ...
    const [inpectionURL, setInspectionURL] = useState('');
    ...
    useFocusEffect(
      ...
      console.log('jujuuuuu',userData.token);

    const getInspectionURL = async () => {
        setLoading(true);
        ...
        fetch("https://gsidev.ordosolution.com/api/send_inspection_pdf_link/", requestOptions)
            .then((response) => response.json())
            .then(async (result) => {
                setInspectionURL(result.message);
                // console.log(result)
                setLoading(false);
            })
            .catch((error) => {
    ...
    const FetchMaintanaData = async () => {
        ...
        await fetch("https://gsidev.ordosolution.com/api/inspections/", requestOptions)
            .then((response) => response.json())
            .then(async (result) => {
    ...
    const applyFilter = () => {
        ...
        const filteredMaintenanceData = originalMaintenanceData.filter(
            (item) => item.Date.split("T")[0] === formattedSelectedDate
        );
    ...
    const renderItem = ({ item }) => {
        ...
        return (
            <TouchableOpacity 
                onPress={() => navigation.navigate('PDFViewer', { url: `https://gsidev.ordosolution.com/api/inspections/${item.id}/pdf` })}
            >
        ...
        >
          ...
          >
            ...
            <AnimatedFAB
                ...
                // iconMode={'static'}
                onPress={() => {
                    navigation.navigate("InspectionForm", { InspectionURL: inpectionURL });
                    // console.log("pressed")
                }}
            />
            <Modal
