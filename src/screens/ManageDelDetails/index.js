...
const ManageDelDetails = ({ navigation, route }) => {
  const {screen,screenNameP ,typeP ,type1P,type2P} = route?.params;
  const ScreenName = route.params?.ScreenName;
  console.log("🚀 ~ ManageDelDetails ~ ScreenName:", ScreenName)
  ...
  const [transportationType, setTransportationType] = useState(details?.transportation_type || '-');
...
console.log("dimension data",dimensionData)
          ...
          await fetch(
            ...
            .then((response) => {
              if (response.status === 200) {
                ...
                navigation.navigate('ManageDeliveries',{screenNameP : screenNameP,typeP :typeP ,type1P :type1P,type2P : type2P })
              }else{
  ...
  const PickUpDelivery= async () => {
    // Display a confirmation dialog
    Alert.alert("Confirm Delivery", "Are you sure you want to Delivery this Order", [
      {
        text: "Cancel",
        onPress: () => console.log("Cancel Pressed"),
        style: "cancel",
      },
      {
        ...
        onPress: async () => {
          ...
          const filteredData = details?.product_list.map((product) => ({
          ...
          const raw = JSON.stringify({
            order_id: details?.id,
            update_remarks: filteredData,
          });

          console.log("<--------> Raw_Charges <------------------>",raw);
          ...
          await fetch(
            `https://gsidev.ordosolution.com/api/update_pickup_status/`,
            requestOptions
          )
            .then((response) => response.json())
            .then((res) => {
              setLoading(false);
              setRemarks1('')
              Toast.show("Delivery successfully", Toast.LONG);
              navigation.navigate('ManageDeliveries',{screenNameP : screenNameP,typeP :typeP ,type1P :type1P,type2P : type2P })
            })
            .catch((error) => {console.log("api error", error);  setLoading(false);});
        },
      },
    ]);
  };
  ...
  const changeStatus = async (status) => {
    ...
    Alert.alert(
      status === "P" ? "Dispatch Order" : "Complete Delivery",
      status === "P"
          ...
          onPress: async () => {
            const handleApiCalls = async () => {
              ...
              try {
                ...
                console.log(
                  "🚀 ~ handleApiCalls ~ secondResponse:",
                  secondResponse
                );
                if (secondResponse.ok) {
                  setModalVisible(false);
                  console.log(
                    "First API call succeeded, now calling the sales_order API"
                  );
  ...
  >
    <View style={{ flex: 1, backgroundColor: "white", padding: 24 }}>
      <View style={styles.header}>
        <TouchableOpacity onPress={() => ScreenName === 'fleet'? navigation.goBack() : navigation.navigate('ManageDeliveries',{screenNameP : screenNameP,typeP :typeP ,type1P :type1P,type2P : type2P })}>
...
{/* 
      ...
      // Fixed an issue by adding a condition where if transportation is Pick-Up ,Signature-Pad is not shown*/}
  ...
  <>
    <View style={styles.row}>
      ...
      <View style={{ flexDirection: 'row', marginTop: 3 }}>
        ...
        {details?.status === "Stock Approved" && ScreenName !== "fleet" && (
                       ...
                       <View
                        ...
                        { details?.is_production === true &&
                              ...
                              <TouchableOpacity style={{
                                ...
                                OpenDimention( ScreenName === "fleet" ? item.dimension : item.production_order)
                              }>
       ...
       {details?.transportation_type === "Delivery" && item?.loaded_bundle ? (
             ...
             {details?.transportation_type === "Pick-Up" && item?.route_loaded_bundle ? (
               <View style={{ flexDirection: "row", alignItems: "center" }}>
                ...
                {
                  item.stock_comments &&
                  <Text style={{color:'black',fontFamily: 'AvenirNextCyr-Medium',fontSize: 13 }}>Stock Remarks: <Text style={{color:'black',fontFamily: 'AvenirNextCyr-Medium' }}>{item.stock_comments}</Text></Text>
                }
                         </View>
...
{(details?.status === "Stock Approved"&&
        ...
        <View style={{ flexDirection: "row", justifyContent: "space-between" ,gap:10}}>
          <TouchableOpacity
            ...
            style={[
              styles.TwoButtons,
              { backgroundColor: "tomato", marginTop: "2%" },
            ]}
          >
            <Text style={styles.Btext}>
            Cancel Order
          ...
          <TouchableOpacity
            ...
            style={[
              styles.TwoButtons,
              { backgroundColor: "orange",marginTop: "2%" },
            ]}
          >
             {isUpdating1 ? 
...
{/* {((details?.status === "Delivery Assigned" &&
        details.transportation_type !== "Delivery") &&
        ScreenName !== "fleet") && (
...
{(details?.status === "Partially Delivered" &&
  ScreenName !== "fleet" &&
  details?.is_production !== true) && (
    <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
      <TouchableOpacity
      ...
      >
        <Text style={styles.Btext}>Close Order</Text>
      </TouchableOpacity>
    </View>
)}
              ...
              >
                ...
                <View
                  style={{
                    flexDirection: "row",
                    paddingVertical: "1%",
                    backgroundColor: selectedOption === "Pick-Up" ? Colors.primary : "white",
                    alignItems: "center",
                    justifyContent: "center",
                    borderRadius: 30,
                    width: "40%"
...
export default ManageDelDetails;
