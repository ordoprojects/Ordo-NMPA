...
const fetchPatients = async () => {
  try {
    ...
    if (response.status === 403) {
      // User doesn't have permission to view dependents (likely a doctor)
    ...
    setShowDependents(false); // Hide section on error too
  }
};
...
console.log("dependents",JSON.stringify(patients,null,2))
...
languageDropdown: {
  marginTop: Platform.OS === 'ios' ? 40 : 0, // Adjust dropdown position for iOS
},
  profileHeader: {
    alignItems: 'center',
    padding: 16,
    marginTop: Platform.OS === 'ios' ? 0 : 0, // Adjust for iOS
  },
  profileImage: {
    width: 128,
    height: 128,
