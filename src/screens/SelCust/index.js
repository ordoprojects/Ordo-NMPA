...
const SelCust= ({ navigation, route }) => {

    const { planName, startDate, endDate, type } = route.params;
    ...
    const getActiveDealerList = async () => {
        ...
        fetch("https://dev.assure.ordosolution.com/get_accounts_visit.php", requestOptions)
    ...
    const createPlanPressed = () => {
        ...
        if (planName && selectedCustomer.length > 0) {
            ...
            console.log("hdhskjhksdhc raww", raw);
            ...
            fetch("https://dev.assure.ordosolution.com/set_tour_plan_v3.php", requestOptions)
                .then(response => response.json())
                .then(result => {
