...
const SupplierReports = ({ navigation }) => {
    ...
    useEffect(() => {
        if (areaChartData && Object.keys(areaChartData).length > 0 && barType && barMode) {
            // console.log("test", totalOrders)
            const data = areaChartData[barType][barMode];
            setFilteredAreaData(data);
        }
    }, [areaChartData, barType, barMode]);

    useEffect(() => {
        if (pos && rfqs && pieType) {
            if (pieType == "purchase_orders") {
                setPieChartData(pos)
            } else {
                setPieChartData(rfqs)
    ...
    const fetchData = async () => {
        ...
        try {
            const response = await fetch("https://gsidev.ordosolution.com/api/supplier_dashboard/", requestOptions);
            const data = await response.json();

            const po_data = data.piechart.purchase_orders.map((item) => ({
                label: item.label,
                value: item.value, // Replace some_property with the property containing the value
                color: getRandomColor(),
            }));

            const rfq_data = data.piechart.rfqs.map((item) => ({
                label: item.label,
                value: item.value,// Replace some_property with the property containing the value
                color: getRandomColor(),
            }));
            ...
            setAreaChartData(data.areachart)
        } catch (error) {
    ...
    return (
        ...
        >
            ...
            <ScrollView style={styles.whiteView}>
                <Graphs pieData={pieChartData} pieChart={true} pieChartHeading="Orders" barChart={true} barGraphHeading="Purchase Orders" barData={barChartData} navigation={navigation} moduleName="Sales" areaChart={true} areaChartData={filteredAreaData} >
                    {/* Custom component as children */}
                    {{
                        areaHeading: <CustomBarHeading selected={barType} />,
                        pieHeading: <CustomPieHeading selected={pieType} />,
                        customAreaSwitch: <CustomSwitch />
                    }}
                </Graphs>
            </ScrollView>
        </LinearGradient>
